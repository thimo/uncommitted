...
`README.md`; for the backlog of ideas see `IDEAS.md`.
...
- **v0.11.0** released. MIT-licensed on GitHub at `thimo/uncommitted`.
  ...
  via `gh`, search + keyboard nav, pending-work age. See `CHANGELOG.md`
...
- `.build/release/UncommittedTests` — custom plain-Swift test runner, not
  XCTest/swift-testing (neither ships with the CLT). Covers
  ...
  aggregation, the per-PR attention classifier (whose turn a PR is, and
